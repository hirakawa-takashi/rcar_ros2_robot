...
- 520 モーターエンコーダーの GPIO 割り付け（`motor_hat.yaml` `encoder` / `gpio_pins.yaml`）は未使用ピンから選んだ暫定値。配線色は端子基板の印字（M+ / M− / VCC / GND / S1 / S2）どおり Motor+ 黒 / Motor− 赤 / VCC 白 / GND 黄 / Encoder A（S1）橙 / Encoder B（S2）緑。公式色表（白 GND / 黄 C1 / 緑 C2 / 青 VCC）は本機のケーブルには適用しない。エンコーダー読み取りノードは未実装
