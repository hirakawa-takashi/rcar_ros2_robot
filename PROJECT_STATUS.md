...
- 520 モーターエンコーダーの GPIO 割り付け（`motor_hat.yaml` `encoder` / `gpio_pins.yaml`）は未使用ピンから選んだ暫定値。配線色は 2026/09/23 にエンコーダー基板の印字（M+ / M− / VCC / GND / S1 / S2）を緑側のコネクタから読み取って確認し、Motor+ 緑 / Motor− 橙 / VCC 黄 / GND 白 / Encoder A（S1）赤 / Encoder B（S2）黒に確定。公式色表（白 GND / 黄 C1 / 緑 C2 / 青 VCC）は本機のケーブルには適用しない。エンコーダー読み取りノードは未実装
