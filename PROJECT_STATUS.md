...
## 最終更新
- 更新日: 2026/09/19
- 更新概要: モーター 6 本の配線色を実物ケーブル（緑・橙・黄・白・赤・黒）に合わせて修正（Motor+ 赤 / Motor− 黒 / VCC 橙 / GND 白 / Encoder A 黄 / Encoder B 緑。旧: Motor− 白 / VCC 青 / GND 黒 / A 緑 / B 黄）。`motor_hat.yaml`・`gpio_pins.yaml`・BOM を更新、接続図・一覧表は再読込で反映。以前: CPU 使用率 20% 以下を目標に、カメラ取り込み 15fps、推論 5Hz、テレメトリ 5Hz、LiDAR 点群 最大360点、IMU 20Hz、autonomy / joy 10Hz、液晶 2Hzへ周期を見直し。実機測定でダッシュボード CPU は概ね 18〜21%まで低下。以前:
...
- 520 モーターエンコーダーの GPIO 割り付け（`motor_hat.yaml` `encoder` / `gpio_pins.yaml`）は未使用ピンから選んだ暫定値。配線色は実物ケーブル（緑・橙・黄・白・赤・黒）に合わせ Motor+ 赤 / Motor− 黒 / VCC 橙 / GND 白 / A 黄 / B 緑 に確定（2026/09/23、公式色表の青 VCC が本ロットでは橙）。エンコーダー読み取りノードは未実装
