...
## 現在作業中
- SPI 液晶（TFT LCD）の接続。配線色は確定済み、RES/DC/BLK のピン割り当ては仮（`config/gpio_pins.yaml` 参照）。表示ノード（コントローラ IC 未確認）は未実装

## 未実装機能
- SPI 液晶表示ノード（コントローラ IC と解像度の確認、`dtparam=spi=on`、`spidev` 権限設定を含む）
...
## 次回作業
- SPI 液晶: 実配線に合わせて `gpio_pins.yaml` の RES/DC/BLK のピンを確定し、コントローラ IC（ST7789 / ILI9341 等）を確認して表示ノードを実装する
...
- `/home/super/AI-CAR_ws/src/ai_car_web/config/gpio_pins.yaml` - SPI 液晶の配線色を追加
