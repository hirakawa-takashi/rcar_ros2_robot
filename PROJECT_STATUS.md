...
## 最終更新
- 更新日: 2026/09/19
- 更新概要: 2026/09/25、プロジェクト説明に「3D プリント」タブ（STL ダウンロード・三面図・立体図・3D 表示・印刷設定・ネジ・組み立て手順）と `GET /print3d/{file}` を追加。以前: 2026/09/25、Motor HAT の出力故障を切り分け、交換を決定（既知の問題に記録）。Motor HAT 配線表に 1 本ごとの「備考」列を追加。エンコーダー VCC / GND の接続先表記を「Motor HAT (3V3 power)」「Motor HAT (Ground)」に統一（ダッシュボード・BOM・設定コメント）。以前: 2026/09/23、モーター 6 本の配線色をエンコーダー基板のコネクタ印字に基づき修正（Motor+ 黒 / Motor− 赤 / VCC 白 / GND 黄 / Encoder A 橙 / Encoder B 緑）。`motor_hat.yaml`・`gpio_pins.yaml`・BOM を更新。以前: 2026/09/23、エンコーダー基板印字（M+ / M− / VCC / GND / S1 / S2）から読み取ったモーター 6 本の配線色を確定（Motor+ 緑 / Motor− 橙 / VCC 黄 / GND 白 / Encoder A 赤 / Encoder B 黒）。`motor_hat.yaml`・`gpio_pins.yaml`・BOM を更新、接続図・一覧表は再読込で反映。以前: CPU 使用率 20% 以下を目標に、カメラ取り込み 15fps、推論 5Hz、テレメトリ 5Hz、LiDAR 点群 最大360点、IMU 20Hz、autonomy / joy 10Hz、液晶 2Hzへ周期を見直し。実機測定でダッシュボード CPU は概ね 18〜21%まで低下。以前:
...
- ai_car_web
  ...
  - 「プロジェクト説明」ボタンと8タブの全画面モーダルを追加（「3D プリント」タブは STL ダウンロード・三面図・立体図・ドラッグで回せる 3D 表示・印刷設定・組み立て手順）。`architecture.yaml` と `/api/architecture` から構成を表示し、`dashboard.yaml` の実値（秘密値はマスク）と実機の生存ノード / Topic を反映
  ...
  - `autonomy_node`（Level 1 自律走行・LiDAR 反応型）: `/scan` を前 / 左 / 右 / 後セクターに分けて最短距離を求め、前進（0.15 m/s）→ 0.8 m 以内で減速（0.08 m/s、空いている側へ緩く曲がる）→ 0.35 m 以内で空いている側へその場旋回（0.6 rad/s、前方 0.9 m 以上空くまで）→ 前進を繰り返す。前後左右とも 0.25 m 以内なら後退。LiDAR が 1 秒以上途絶えると停止。`/cmd_vel_auto` に 10Hz で常時 publish（AUTO 以外はゼロ速度で待機）、状態は `/autonomy_status`（JSON: behavior / sectors / cmd）。地図・自己位置は使わない。launch 引数 `use_autonomy`
  ...
  - ゲームパッド手動操作（`joy_teleop_node`）: Logitech F710（XInput モード、`/dev/input/js0`）を Linux joystick API で直接読み（追加の Python 依存なし）、正規化済み Twist を `/joy_cmd` に 10Hz で publish。`dashboard_node` が `/joy_cmd` を購読して Web 操作と同じ `publish_cmd_vel()` 経路で `/cmd_vel` に変換するため、最大速度・障害物ガード（減速／停止）・`cmd_timeout` 停止が同様に適用される。割り当て: 左スティック上下=前後、左スティック左右=平行移動、右スティック左右=旋回、十字キー=前後・平行移動（デジタル）、RB=全速（通常は `speed_scale` 0.5）、B=停止（押している間。自動中は STOP モード要求）、START 長押し 2 秒=手動 ⇄ 自動、BACK=STOP モード、`button_enable` でデッドマンボタンも設定可（既定は無効）。F710 未接続でも起動し、抜き差しを検出して再接続（切断時は指令を出さない）。画面の手動操作カードに接続状態（未接続／接続中／操作中）を表示。launch 引数 `use_joy` で無効化可
...
- Motor HAT（Adafruit DC & Stepper Motor HAT）がモーターを駆動しない。2026/09/25 に切り分け、故障と判断して交換を決定。12V は電源ピン 11.94V・HAT 内コンデンサ 12V まで到達し極性も正しいが、緑 LED（電源表示）が点灯しない。Pi から I2C 0x60 に通信でき、駆動中に読み出した PCA9685 の値は M1（ch 8=約 50% / IN1 ch 10 全 ON / IN2 ch 9 全 OFF）・M3（ch 2 / 4 / 3 同様）とも正しく、MODE1=0x01（スリープなし）、ALL_LED 全 OFF なし、PRESCALE=0x03（約 1.5kHz）、`get_throttled=0x0`。それでも 50% で 10 秒駆動中の M1・M3 の出力端子はテスターで 0V（TB6612 #1・#2 とも出力なし）。モーター単体は 12V 直結で回転し、手回しで M1 ピンに ±2V の発電電圧が出るため、モーター・線・端子の接触は正常。2 個の TB6612 が同時に出力せず LED も消灯していることから、共通の電源経路またはドライバーのロジック電源側の故障と推定（故障部品は未特定）。交換品は同型（Adafruit 製品番号 2348、I2C 0x60、M1〜M4 割り付け同じ）で、ソフト・配線表の変更は不要。交換後はまず 12V のみで緑 LED 点灯を確認し、M1 を 30% で短時間駆動して確認する

## テスト結果
- `lcd_display_node`（実機、2026/09/15）: `python3-spidev` / `fonts-ipafont-gothic` を apt 導入、`/dev/spidev0.0` は dialout グループで super から書き込み可。サービス再起動後 `lcd_display_node` が起動し「ST7789 液晶表示を接続しました」、`/display_state` に `rdy ` を publish。`seg_display_node` は起動していないことを確認。2026/09/16 に配線（GND 黒 / VCC 赤 / SCL 橙 / SDA 黄 / RES 緑 / DC 青 / BLK 紫）後、ノード再起動で **正常表示を確認**（SPI モード 3、32MHz、rotation 270 = 反時計回り 90°、横 80px オフセット）。2026/09/17 に更に反時計回り 90° 回転させ rotation 180（縦 80px オフセット）に変更し、実機で **要望通りの向きで正常表示を確認**。配線後はパネル初期化のためノード再起動が必要（起動中に配線しても表示されない）
...
## 次回作業
- Motor HAT を同型品に交換し、12V 投入で緑 LED 点灯 → M1〜M4 を 30% で短時間駆動して出力と回転を確認する
...
- `/home/super/AI-CAR_ws/src/ai_car_web/{ai_car_web/architecture.py, ai_car_web/dashboard_node.py, config/architecture.yaml, config/dashboard.yaml, static/index.html}` - プロジェクト説明モーダルと `/api/architecture`
