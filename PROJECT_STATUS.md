...
- 520 モーターエンコーダーの GPIO 割り付け（`motor_hat.yaml` `encoder` / `gpio_pins.yaml`）は未使用ピンから選んだ暫定値。配線色は端子基板の印字（M+ / M− / VCC / GND / S1 / S2）とテスター実測から Motor+ 黒 / Motor− 赤 / VCC 黄 / GND 白 / Encoder A（S1）橙 / Encoder B（S2）緑（Motor+/− は黒-赤間 4Ω・手回し発電で確認、VCC / GND は 1kΩ 直列給電で黄を＋にすると約 0.5mA 流れることを確認し、印字の表の白 VCC / 黄 GND とは逆。橙・緑のパルス出力は 3.3V 給電で手回ししても 2.7V のまま変化せず未確認）。公式色表（白 GND / 黄 C1 / 緑 C2 / 青 VCC）は本機のケーブルには適用しない。エンコーダー読み取りノードは未実装
