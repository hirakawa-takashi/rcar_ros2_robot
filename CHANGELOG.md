...
### Added
- モーター 6 本の配線色を端子基板の印字とテスター実測に基づき修正: Motor+（M+）黒 / Motor−（M−）赤 / VCC 黄 / GND 白 / Encoder A（S1）橙 / Encoder B（S2）緑（VCC / GND は印字の表と逆だったため実測を優先。旧: Motor+ 緑 / Motor− 橙 / VCC 黄 / GND 白 / A 赤 / B 黒）。`motor_hat.yaml`・`gpio_pins.yaml`・`HARDWARE_BOM.md` を更新
