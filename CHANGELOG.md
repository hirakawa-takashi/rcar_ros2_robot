...
### Added
- 落下防止センサー（VL53L1X ×2）を配線図とプログラムに反映。配線は 前: VIN 1 番・GND 14 番・XSHUT 11 番（GPIO17）、後ろ: VIN 17 番・GND 25 番・XSHUT 13 番（GPIO27）、SDA / SCL は 3・5 番を IMU と共有（`gpio_pins.yaml`）。新ノード `cliff_sensor_node` が XSHUT で 1 個ずつ起こしてアドレスを 0x2A / 0x2B に変え（BNO055 の 0x29 と重ならない）、短距離モード・約 40 回/秒で距離を読み、130 mm を超えるか反射が返らない状態が 2 回続いたら段差として `/cliff_status` に publish。`drive_mode_node` は前の段差で前進、後ろの段差で後退を 0 にし、AUTO 中は STOP、AUTO への切替も拒否する（`cliff_guard` / `cliff_timeout` / `cliff_required`）。実機では未確認。モーター制御ノードが未実装のため、実際のモーターはまだ止まらない
- バッテリー本体の残量表示（%）をカバーの外から見られるよう、ふた（15 × 14 mm）と上面カバー（17 × 16 mm、縁は 1 mm 面取り）に窓を開けた。位置は上のバッテリーの表示の実測（ポートの短辺から 7〜20 mm、右の長辺から 9 mm〜）に合わせた（`bat_disp`）。窓の上を通っていたカメラのフラットケーブルは、ふたの上で 45° に 2 回折って左へ 30 mm ずらし（後ろ半分は X 75〜91 mm、`fpc_rx`）、ふたの後ろ端から下りたあと DROK の上で右へ戻す。ブリッジは Y 168 mm（前半）と X 83・Y 128 mm（後ろ半分）へ移動。ケーブルの必要な長さは約 22 cm → 約 28 cm（300 mm で足りる）。LiDAR の USB 変換基板はふたの上の左寄りに貼る。`lid.stl`・`cover.stl` と組み立て図・配線図・カバーの画像を更新し、`battery_display.png` を追加
- 印刷の材料を PLA に合わせて、バンパーの板ばねを 1.2 mm から 0.8 mm に薄くした（4 mm 逃げたときのひずみ 1.6 % → 1.1 %、PLA の降伏ひずみ約 2 % の半分ほど）。ストッパーは 6.8 mm、前面の板は 0.4 mm 後ろへ。カバーのフック（ひずみ約 0.9 %）はそのまま。`bumper.stl` を更新し、材料の説明を PLA に変更
